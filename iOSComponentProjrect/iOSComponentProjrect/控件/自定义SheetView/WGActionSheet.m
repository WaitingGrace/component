...
- (instancetype)initWithDelegate:(id<WGActionSheetDelegate>)delegate cancelButtonTitle:(NSString *)cancelButtonTitle destructiveButtonTitle:(NSString *)destructiveButtonTitle otherButtonTitles:(NSString *)otherButtonTitles,... {
    self = [super initWithFrame:CGRectZero];
    if (self) {
        [self __commonInit];
...
- (NSInteger)addButtonWithTitle:(NSString *)title actionColor:(UIColor *)actionColor{
    ...
    [newButton setFrame:CGRectMake(0, 0, self.screenSize.width, kButtonHeight)];
    [newButton setTitle:title forState:UIControlStateNormal];
    [newButton setTitleColor:actionColor forState:(UIControlStateNormal)];
    [newButton addTarget:self action:@selector(dismissWithClickedButton:) forControlEvents:UIControlEventTouchUpInside];
    NSUInteger index = [self.buttons count];
...
- (void)dismissWithClickedButton:(UIButton *)button {
    ...
    void (^actionBlockForButton)(void) = [self.actionBlockForButtonIndex objectForKey:[NSNumber numberWithInteger:buttonIndex]];
