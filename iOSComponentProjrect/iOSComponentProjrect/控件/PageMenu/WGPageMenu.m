...
- (instancetype)initWithImagePosition:(WGItemImagePosition)imagePosition;

@property (nonatomic) WGItemImagePosition imagePosition; // 图片位置
...
- (void)initialize {
    _imagePosition = WGItemImagePositionLeft;
...
- (CGRect)titleRectForContentRect:(CGRect)contentRect {
    ...
    switch (self.imagePosition) {
        case WGItemImagePositionLeft:
        case WGItemImagePositionDefault: {
...
- (void)setImagePosition:(WGItemImagePosition)imagePosition {
...
@interface WGPageMenu()
@property (nonatomic, assign) WGPageMenuTrackerStyle trackerStyle;
...
@implementation WGPageMenu


#pragma mark - public

+ (instancetype)pageMenuWithFrame:(CGRect)frame trackerStyle:(WGPageMenuTrackerStyle)trackerStyle {
    WGPageMenu *pageMenu = [[WGPageMenu alloc] initWithFrame:frame trackerStyle:trackerStyle];
    return pageMenu;
}

- (instancetype)initWithFrame:(CGRect)frame trackerStyle:(WGPageMenuTrackerStyle)trackerStyle {
    if (self = [super init]) {
        self.frame = frame;
...
- (void)setItems:(NSArray *)items selectedItemIndex:(NSInteger)selectedItemIndex {
    ...
    for (int i = 0; i < items.count; i++) {
        id object = items[i];
        NSAssert([object isKindOfClass:[NSString class]] || [object isKindOfClass:[UIImage class]] || [object isKindOfClass:[WGPageMenuButtonItem class]], @"items中的元素类型只能是NSString、UIImage或SPPageMenuButtonItem");
...
- (void)insertItem:(WGPageMenuButtonItem *)item atIndex:(NSUInteger)itemIndex animated:(BOOL)animated {
    self.insert = YES;
    NSAssert(itemIndex <= self.items.count, @"itemIndex超过了items的总个数“%ld”",self.items.count);
    NSMutableArray *objects = self.items.mutableCopy;
...
- (void)setItem:(WGPageMenuButtonItem *)item forItemIndex:(NSUInteger)itemIndex {
    if (item == nil) return;
    if (itemIndex < self.buttons.count) {
        SPPageMenuButton *button = [self.buttons objectAtIndex:itemIndex];
...
- (void)setItem:(WGPageMenuButtonItem *)item forItemAtIndex:(NSUInteger)itemIndex {
    [self setItem:item forItemIndex:itemIndex];
}

- (WGPageMenuButtonItem *)itemAtIndex:(NSUInteger)itemIndex {
    if (itemIndex < self.items.count) {
        id object = [self.items objectAtIndex:itemIndex];
        NSAssert([object isKindOfClass:[WGPageMenuButtonItem class]],@"itemIndex对应的item不是SPPageMenuButtonItem类型，请仔细核对");
        return object;
    }
    return nil;
}
...
- (void)setContent:(id)content forItemIndex:(NSUInteger)itemIndex {
    ...
    if (itemIndex < self.buttons.count) {
        ...
        } else if ([content isKindOfClass:[WGPageMenuButtonItem class]]) {
            WGPageMenuButtonItem *item = (WGPageMenuButtonItem *)content;
            [button setTitle:item.title forState:UIControlStateNormal];
            [button setImage:item.image forState:UIControlStateNormal];
            button.imagePosition = item.imagePosition;
            button.imageTitleSpace = item.imageTitleSpace;
...
- (void)setFunctionButtonWithItem:(WGPageMenuButtonItem *)item forState:(UIControlState)state {
...
- (void)setTitle:(nullable NSString *)title image:(nullable UIImage *)image imagePosition:(WGItemImagePosition)imagePosition imageRatio:(CGFloat)ratio forItemIndex:(NSUInteger)itemIndex {
    if (itemIndex < self.buttons.count) {
        SPPageMenuButton *button = [self.buttons objectAtIndex:itemIndex];
        [button setTitle:title forState:UIControlStateNormal];
        [button setImage:image forState:UIControlStateNormal];
...
- (void)setTitle:(nullable NSString *)title image:(nullable UIImage *)image imagePosition:(WGItemImagePosition)imagePosition imageRatio:(CGFloat)ratio imageTitleSpace:(CGFloat)imageTitleSpace forItemIndex:(NSUInteger)itemIndex {
    if (itemIndex < self.buttons.count) {
        SPPageMenuButton *button = [self.buttons objectAtIndex:itemIndex];
        [button setTitle:title forState:UIControlStateNormal];
        [button setImage:image forState:UIControlStateNormal];
...
- (void)setFunctionButtonTitle:(nullable NSString *)title image:(nullable UIImage *)image imagePosition:(WGItemImagePosition)imagePosition imageRatio:(CGFloat)ratio forState:(UIControlState)state {
...
- (void)setFunctionButtonTitle:(nullable NSString *)title image:(nullable UIImage *)image imagePosition:(WGItemImagePosition)imagePosition imageRatio:(CGFloat)ratio imageTitleSpace:(CGFloat)imageTitleSpace forState:(UIControlState)state {
    [self.functionButton setTitle:title forState:state];
    [self.functionButton setImage:image forState:state];
    self.functionButton.imagePosition = imagePosition;
...
- (void)addButton:(NSInteger)index object:(id)object animated:(BOOL)animated {
    ...
    if ([object isKindOfClass:[NSString class]]) {
        ...
        WGPageMenuButtonItem *item = (WGPageMenuButtonItem *)object;
        [button setTitle:item.title forState:UIControlStateNormal];
        [button setImage:item.image forState:UIControlStateNormal];
        button.imagePosition = item.imagePosition;
        button.imageTitleSpace = item.imageTitleSpace;
...
- (BOOL)haveOrNeedsTracker {
    if (self.trackerStyle != WGPageMenuTrackerStyleTextZoom && self.trackerStyle != WGPageMenuTrackerStyleNothing) {
        return YES;
    }
    return NO;
}
...
- (void)buttonInPageMenuClicked:(SPPageMenuButton *)sender {
    ...
    if (self.selectedButton != sender) {
        ...
        if (self.trackerStyle == WGPageMenuTrackerStyleTextZoom || _selectedItemZoomScale != 1) {
...
- (void)prepareMoveTrackerFollowScrollView:(UIScrollView *)scrollView {
    ...
    switch (self.trackerFollowingMode) {
        case WGPageMenuTrackerFollowingModeAlways:
            // 这个方法才开始移动跟踪器
            ...
            break;
        case WGPageMenuTrackerFollowingModeHalf:{
...
- (void)moveTrackerWithProgress:(CGFloat)progress fromIndex:(NSInteger)fromIndex toIndex:(NSInteger)toIndex currentOffsetX:(CGFloat)currentOffsetX beginOffsetX:(CGFloat)beginOffsetX {
    ...
    if (self.trackerStyle == WGPageMenuTrackerStyleLine) {
    ...
    } else if (self.trackerStyle == WGPageMenuTrackerStyleLineAttachment) {
    ...
    } else if (self.trackerStyle == WGPageMenuTrackerStyleTextZoom || self.trackerStyle == WGPageMenuTrackerStyleNothing) {
        // 缩放文字
        if (_selectedItemZoomScale != 1) {
            [self zoomForTitleWithProgress:progress fromButton:fromButton toButton:toButton];
        }
    } else if (self.trackerStyle == WGPageMenuTrackerStyleRoundedRect) {
        newCenter.x = fromButton.center.x + xDistance * progress;
...
- (void)setTrackerStyle:(WGPageMenuTrackerStyle)trackerStyle {
    _trackerStyle = trackerStyle;
    switch (trackerStyle) {
        case WGPageMenuTrackerStyleLine:
        case WGPageMenuTrackerStyleLineLongerThanItem:
        case WGPageMenuTrackerStyleLineAttachment:
            self.tracker.backgroundColor = _selectedItemTitleColor;
            break;
        case WGPageMenuTrackerStyleRoundedRect:
        case WGPageMenuTrackerStyleRect:
            ...
            _trackerHeight = 0;
            break;
        case WGPageMenuTrackerStyleTextZoom:
            // 此样式下默认1.3
...
- (void)setDelegate:(id<WGPageMenuDelegate>)delegate {
...
- (void)setPermutationWay:(WGPageMenuPermutationWay)permutationWay {
    _permutationWay = permutationWay;
    if (!_forceUseSettingSpacing) {
        if (_permutationWay == WGPageMenuPermutationWayNotScrollEqualWidths) {
...
- (void)setCloseTrackerFollowingMode:(BOOL)closeTrackerFollowingMode {
    ...
    if (closeTrackerFollowingMode) {
        self.trackerFollowingMode = WGPageMenuTrackerFollowingModeEnd;
    } else {
        self.trackerFollowingMode = WGPageMenuTrackerFollowingModeAlways;
    }
}
#pragma mark - getter
...
- (void)layoutSubviews {
    ...
    for (int i= 0 ; i < self.buttons.count; i++) {
        ...
        } else if (button.currentTitle.length && button.currentImage && (button.imagePosition == WGItemImagePositionRight || button.imagePosition == WGItemImagePositionLeft || button.imagePosition == WGItemImagePositionDefault)) {
            contentW = textW + imageW + button.imageTitleSpace+button.contentEdgeInsets.left+button.contentEdgeInsets.right;
        } else if (button.currentTitle && button.currentImage && (button.imagePosition == WGItemImagePositionTop || button.imagePosition == WGItemImagePositionBottom)) {
    ...
    if (self.permutationWay == WGPageMenuPermutationWayNotScrollAdaptContent && diff < 0) {
    ...
    [self.buttons enumerateObjectsUsingBlock:^(SPPageMenuButton *button, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (self.permutationWay == WGPageMenuPermutationWayScrollAdaptContent) {
        ...
        } else if (self.permutationWay == WGPageMenuPermutationWayNotScrollEqualWidths) {
            // 求出外界设置的按钮宽度之和
...
@implementation WGPageMenuButtonItem

+ (instancetype)itemWithTitle:(NSString *)title image:(UIImage *)image {
    WGPageMenuButtonItem *item = [[WGPageMenuButtonItem alloc] initWithTitle:title image:image imagePosition:WGItemImagePositionDefault];
    return item;
}

+ (instancetype)itemWithTitle:(NSString *)title image:(UIImage *)image imagePosition:(WGItemImagePosition)imagePosition {
    WGPageMenuButtonItem *item = [[WGPageMenuButtonItem alloc] initWithTitle:title image:image imagePosition:imagePosition];
    return item;
}

- (instancetype)initWithTitle:(NSString *)title image:(UIImage *)image imagePosition:(WGItemImagePosition)imagePosition {
    if (self = [super init]) {
        self.title = title;
        self.image = image;
        self.imagePosition = imagePosition;
