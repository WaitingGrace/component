...
typedef NS_ENUM(NSInteger, WGPageMenuTrackerStyle) {
    WGPageMenuTrackerStyleLine = 0,                  // 下划线,默认与item等宽
    WGPageMenuTrackerStyleLineLongerThanItem,        // 下划线,比item要长(长度为item的宽+间距)
    WGPageMenuTrackerStyleLineAttachment,            // 下划线“依恋”样式，此样式下默认宽度为字体的pointSize，你可以通过trackerWidth自定义宽度
    WGPageMenuTrackerStyleRoundedRect,               // 圆角矩形
    WGPageMenuTrackerStyleRect,                      // 矩形
    WGPageMenuTrackerStyleTextZoom NS_ENUM_DEPRECATED_IOS(6_0, 6_0, "该枚举值已经被废弃，请用“selectedItemZoomScale”属性代替"), // 缩放(该枚举已经被废弃,用属性代替的目的是让其余样式可与缩放样式配套使用。如果你同时设置了该枚举和selectedItemZoomScale属性，selectedItemZoomScale优先级高于SPPageMenuTrackerStyleTextZoom
    WGPageMenuTrackerStyleNothing                    // 什么样式都没有
};

typedef NS_ENUM(NSInteger, WGPageMenuPermutationWay) {
    WGPageMenuPermutationWayScrollAdaptContent = 0,  // 自适应内容,可以左右滑动
    WGPageMenuPermutationWayNotScrollEqualWidths,    // 等宽排列,不可以滑动,整个内容被控制在pageMenu的范围之内,等宽是根据pageMenu的总宽度对每个按钮均分
    WGPageMenuPermutationWayNotScrollAdaptContent    // 自适应内容,不可以滑动,整个内容被控制在pageMenu的范围之内,这种排列方式下,自动计算item之间的间距,itemPadding属性无效
};

typedef NS_ENUM(NSInteger, WGPageMenuTrackerFollowingMode) {
    WGPageMenuTrackerFollowingModeAlways = 0,   // 外界scrollView拖动时，跟踪器时刻跟随外界scrollView移动
    WGPageMenuTrackerFollowingModeEnd,     // 外界scrollVie拖动结束后，跟踪器才开始移动
    WGPageMenuTrackerFollowingModeHalf     // 外界scrollView拖动到一半时，跟踪器开始移动
};

typedef NS_ENUM(NSInteger, WGItemImagePosition) {
    WGItemImagePositionDefault,   // 默认图片在左侧
    WGItemImagePositionLeft,      // 图片在文字左侧
    WGItemImagePositionRight,     // 图片在文字右侧
    WGItemImagePositionTop,       // 图片在文字上侧
    WGItemImagePositionBottom     // 图片在文字下侧
};

@class WGPageMenu,WGPageMenuButtonItem;

@protocol WGPageMenuDelegate <NSObject>

@optional
// 若以下2个代理方法同时实现了，只会走第2个代理方法（第2个代理方法包含了第1个代理方法的功能）
- (void)pageMenu:(WGPageMenu *)pageMenu itemSelectedAtIndex:(NSInteger)index;
- (void)pageMenu:(WGPageMenu *)pageMenu itemSelectedFromIndex:(NSInteger)fromIndex toIndex:(NSInteger)toIndex;

// 右侧的功能按钮被点击的代理方法
- (void)pageMenu:(WGPageMenu *)pageMenu functionButtonClicked:(UIButton *)functionButton;
@end

@interface WGPageMenu : UIView
...
#else
@property (nonatomic, readonly) WGPageMenuTrackerStyle trackerStyle;
#endif

@property (nonatomic, assign) WGPageMenuPermutationWay permutationWay; // 排列方式
...
@property (nonatomic, assign) WGPageMenuTrackerFollowingMode trackerFollowingMode; // 跟踪器的跟踪模式
...
@property (nonatomic, weak) id<WGPageMenuDelegate> delegate;
...
- (void)insertItem:(nonnull WGPageMenuButtonItem *)item atIndex:(NSUInteger)itemIndex animated:(BOOL)animated;
...
- (void)setItem:(WGPageMenuButtonItem *)item forItemAtIndex:(NSUInteger)itemIndex; // 同时为指定item设置标题和图片
- (nullable WGPageMenuButtonItem *)itemAtIndex:(NSUInteger)itemIndex; // 获取指定item
...
- (void)setTitle:(nullable NSString *)title image:(nullable UIImage *)image imagePosition:(WGItemImagePosition)imagePosition imageRatio:(CGFloat)ratio forItemIndex:(NSUInteger)itemIndex NS_DEPRECATED_IOS(6_0, 6_0, "Use -setItem: forItemIndex:");
- (void)setTitle:(nullable NSString *)title image:(nullable UIImage *)image imagePosition:(WGItemImagePosition)imagePosition imageRatio:(CGFloat)ratio imageTitleSpace:(CGFloat)imageTitleSpace forItemIndex:(NSUInteger)itemIndex NS_DEPRECATED_IOS(6_0, 6_0, "Use -setItem: forItemIndex:");
- (void)setFunctionButtonTitle:(nullable NSString *)title image:(nullable UIImage *)image imagePosition:(WGItemImagePosition)imagePosition imageRatio:(CGFloat)ratio forState:(UIControlState)state NS_DEPRECATED_IOS(6_0, 6_0, "Use - setFunctionButtonWithItem:forState:");
- (void)setFunctionButtonTitle:(nullable NSString *)title image:(nullable UIImage *)image imagePosition:(WGItemImagePosition)imagePosition imageRatio:(CGFloat)ratio imageTitleSpace:(CGFloat)imageTitleSpace forState:(UIControlState)state NS_DEPRECATED_IOS(6_0, 6_0, "Use - setFunctionButtonWithItem:forState:");
- (id)objectForItemAtIndex:(NSUInteger)itemIndex NS_DEPRECATED_IOS(6_0, 6_0, "Use -contentForItemAtIndex:");
- (void)setFunctionButtonWithItem:(WGPageMenuButtonItem *)item forState:(UIControlState)state NS_DEPRECATED_IOS(6_0, 6_0, "Use -setFunctionButtonContent:forState:");
- (void)setItem:(WGPageMenuButtonItem *)item forItemIndex:(NSUInteger)itemIndex NS_DEPRECATED_IOS(6_0, 6_0, "Use -setItem:forItemAtIndex:");
...
@interface WGPageMenuButtonItem : NSObject
...
// 图片的位置
@property (nonatomic, assign) WGItemImagePosition imagePosition;
// 图片与标题之间的间距,默认0.0
