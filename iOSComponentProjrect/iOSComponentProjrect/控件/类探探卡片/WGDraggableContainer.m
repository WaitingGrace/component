...
@interface WGDraggableContainer ()
...
@implementation WGDraggableContainer
...
- (void)defaultConfig {
    ...
    self.direction = WGDraggableDirectionDefault;
    self.loadedIndex = 0;
    self.moving = NO;
}
    ...
    if (self.dataSource && [self.dataSource respondsToSelector:@selector(numberOfIndexs)] && [self.dataSource respondsToSelector:@selector(draggableContainer:viewForIndex:)]) {
        ...
        if (self.loadedIndex < indexs) {
            ...
            for (long int i = self.currentCards.count; i <  (self.moving ? preloadViewCont + 1: preloadViewCont); i++) {
                ...
                WGDraggableCardView *cardView = [self.dataSource draggableContainer:self viewForIndex:self.loadedIndex];
...
- (void)tapGestureHandle:(UITapGestureRecognizer *)tap {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(draggableContainer:cardView:didSelectIndex:)]) {
        [self.delegate draggableContainer:self cardView:(WGDraggableCardView *)tap.view didSelectIndex:tap.view.tag];
    }
}
...
- (void)panGestureHandle:(UIPanGestureRecognizer *)gesture {
    ...
    if (gesture.state == UIGestureRecognizerStateChanged) {
        ...
        WGDraggableCardView *cardView = (WGDraggableCardView *)gesture.view;
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(draggableContainer:draggableDirection:widthRatio:heightRatio:)]) {
            ...
            if (widthRatio > 0) {
                self.direction = WGDraggableDirectionRight;
            } if (widthRatio < 0) {
                self.direction = WGDraggableDirectionLeft;
...
- (void)finishedPanGesture:(UIView *)cardView direction:(WGDraggableDirection)direction scale:(CGFloat)scale disappear:(BOOL)disappear {
    ...
    if (!disappear) {
        ...
        NSInteger flag = direction == WGDraggableDirectionLeft ? -1 : 2;
        [UIView animateWithDuration:0.5f
                              delay:0.0
                            options:UIViewAnimationOptionCurveLinear | UIViewAnimationOptionAllowUserInteraction
                         animations:^{
                             cardView.center = CGPointMake(ScreenWidth * flag, ScreenWidth * flag / scale + self.cardCenter.y);
...
- (void)removeFormDirection:(WGDraggableDirection)direction {
    ...
    if (self.moving) {
        ...
        switch (direction) {
            case WGDraggableDirectionLeft:
                cardCenter = CGPointMake(-ScreenWidth / 2, self.cardCenter.y);
                flag = -1;
                break;
            case WGDraggableDirectionRight:
...
- (void)resetVisibleCards {
    ...
    __weak WGDraggableContainer *weakself = self;
...
- (void)movingVisibleCards:(CGFloat)scale {
    ...
    for (int i = 1; i < self.currentCards.count; i++) {
        ...
        WGDraggableCardView *cardView = [self.currentCards objectAtIndex:i];
