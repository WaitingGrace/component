...
@interface WGBirthdayUserDescView () <UICollectionViewDelegateFlowLayout,UICollectionViewDataSource>
...
@implementation WGBirthdayUserDescView

static NSString * cellIdentifier = @"WGBirthdayUserFriendsCollectionViewCell";
- (instancetype)initWithFrame:(CGRect)frame birthdayModel:(WGBirthdayModel *)birthdayModel {
...
- (void)setupSubviews:(CGRect)frame birthdayModel:(WGBirthdayModel *)birthdayModel {
    self.uiScale = 1;
    CGFloat orginX = 28.0 * self.uiScale;
    CGFloat orginY = birthdayModel.birthdayLayerType == WGBirthdayLayerTypeForA ? 50.0 *self.uiScale : 43.0 *self.uiScale;
    ...
    respectLabel.textColor = UIColorFromRGB(0xff781a22);
    ...
    CGFloat descTopMargin = birthdayModel.birthdayLayerType == WGBirthdayLayerTypeForA ? 12 *self.uiScale : 10 *self.uiScale;
    ...
    descLabel.textColor = UIColorFromRGB(0xff781a22);
    CGFloat lineSpacing = birthdayModel.birthdayLayerType == WGBirthdayLayerTypeForA ? 10 *self.uiScale : 5 *self.uiScale;
    descLabel.attributedText = [self getAttributeText:birthdayModel.birthdayLayerDesc lineSpacing:lineSpacing];
    [descLabel sizeToFit];
    [self addSubview:descLabel];
    if (WGBirthdayLayerTypeForB == birthdayModel.birthdayLayerType) {
        self.friendsList = birthdayModel.friendsBirthdayInfo;
        if (self.friendsList.count > 3) {
            WGBirthdayFriendsItem *friendItem = self.friendsList[3];
        ...
        [collectionView registerNib:[UINib nibWithNibName:@"WGBirthdayUserFriendsCollectionViewCell" bundle:nil] forCellWithReuseIdentifier:cellIdentifier];
    }
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    WGBirthdayUserFriendsCollectionViewCell * cell = (WGBirthdayUserFriendsCollectionViewCell *)[collectionView dequeueReusableCellWithReuseIdentifier:cellIdentifier forIndexPath:indexPath];
    NSInteger item = indexPath.item;
    if (item < self.friendsList.count) {
        WGBirthdayFriendsItem *friendsItem = self.friendsList[item];
