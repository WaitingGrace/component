...
#import "WGOperationSuccessHUD.h"
...
@implementation WGOperationSuccessHUD {
...
+ (WGOperationSuccessHUD*)showIn:(UIView*)view {
    [self hideIn:view];
    WGOperationSuccessHUD *hud = [[WGOperationSuccessHUD alloc] initWithFrame:view.bounds];
    [hud start];
    [view addSubview:hud];
    return hud;
}
...
+ (WGOperationSuccessHUD *)hideIn:(UIView *)view {
    WGOperationSuccessHUD *hud = nil;
    for (WGOperationSuccessHUD *subView in view.subviews) {
        if ([subView isKindOfClass:[WGOperationSuccessHUD class]]) {
            [subView hide];
