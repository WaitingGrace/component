...
+(WGOperationLoadingHUD*)showIn:(UIView*)view{
    [self hideIn:view];
    WGOperationLoadingHUD *hud = [[WGOperationLoadingHUD alloc] initWithFrame:view.bounds];
    [hud start];
    [view addSubview:hud];
    return hud;
}
...
+(WGOperationLoadingHUD *)hideIn:(UIView *)view{
    WGOperationLoadingHUD *hud = nil;
    for (WGOperationLoadingHUD *subView in view.subviews) {
        if ([subView isKindOfClass:[WGOperationLoadingHUD class]]) {
            [subView hide];
