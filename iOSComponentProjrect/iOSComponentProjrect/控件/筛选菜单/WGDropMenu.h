...
@class WGDropMenu,WGDropMenuModel;
@protocol WGDropMenuDataSource <NSObject>
@required
/** 配置筛选菜单标题 */
...
@end
@protocol WGDropMenuDelegate <NSObject>
@optional
/**
 代理回调
...
dropMenuModel: (nullable WGDropMenuModel *)dropMenuModel
 ...
 */
- (void)dropMenu: (WGDropMenu *)dropMenu
dropMenuTitleModel: (nullable WGDropMenuModel *)dropMenuTitleModel;
...
typedef void(^DropMenuTitleBlock)(WGDropMenuModel *dropMenuModel);
typedef void(^DropMenuTagArrayBlock)(NSArray *tagArray);
typedef void(^DropMenuFinishBlock)(void);

@interface WGDropMenu : UIView
/**
 构造WGDropMenu
 ...
 */
+ (instancetype)creatDropMenuWithConfiguration: (nullable WGDropMenuModel *)configuration
                                         frame: (CGRect)frame
...
@property (nonatomic , weak) id <WGDropMenuDelegate> delegate;
@property (nonatomic , weak) id <WGDropMenuDataSource> dataSource;
/** 动画时间 等于0 不开启动画 默认是0 */
