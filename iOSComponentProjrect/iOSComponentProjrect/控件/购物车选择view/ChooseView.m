...
-(void)creatUI{
    ...
    //装载商品信息的视图
    whiteView = [[UIView alloc] initWithFrame:CGRectMake(0, 280, ScreenWidth, ScreenHeight-280)];
    ...
    cancelBtn.frame = CGRectMake(ScreenWidth-40, 10, 25, 25);
...
//    [whiteView addSubview:LB_showSales];
    ...
    //分界线
    LB_line = [[UILabel alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(headImage.frame)+10, ScreenWidth, 0.5)];
    LB_line.backgroundColor = BackColor;
    [whiteView addSubview:LB_line];
    
    //加入购物车按钮
    addBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    addBtn.frame = CGRectMake(0, whiteView.frame.size.height-40, whiteView.frame.size.width, 40);
    
    [addBtn setBackgroundColor:BASEColor];
    [addBtn setTitleColor:[UIColor whiteColor] forState:0];
    addBtn.titleLabel.font = [UIFont systemFontOfSize:15];
    [addBtn setTitle:@"确定" forState:0];
...
//    stockBtn.hidden = YES;
    ...
    mainscrollview = [[UIScrollView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(headImage.frame)+10, ScreenWidth, whiteView.frame.size.height-CGRectGetMaxY(headImage.frame)+10-60)];
