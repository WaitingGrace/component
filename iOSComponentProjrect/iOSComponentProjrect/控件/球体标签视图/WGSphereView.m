...
@interface WGSphereView()<UIGestureRecognizerDelegate>
{
    NSMutableArray *tags;
    NSMutableArray *coordinate;
    WGPoint normalDirection;
    CGPoint last;
...
- (void)setItems:(NSArray *)items
{
    ...
    for (NSInteger i = 0; i < tags.count; i ++) {
        ...
        WGPoint point = WGPointMake(x, y, z);
        NSValue *value = [NSValue value:&point withObjCType:@encode(WGPoint)];
    ...
    normalDirection = WGPointMake(a, b, 0);
    [self timerStart];
}

#pragma mark - set frame of point

- (void)updateFrameOfPoint:(NSInteger)index direction:(WGPoint)direction andAngle:(CGFloat)angle
{
    ...
    WGPoint rPoint = WGPointMakeRotation(point, direction, angle);
    value = [NSValue value:&rPoint withObjCType:@encode(WGPoint)];
...
- (void)setTagOfPoint: (WGPoint)point andIndex:(NSInteger)index
{
...
- (void)handlePanGesture:(UIPanGestureRecognizer *)gesture
{
    if (gesture.state == UIGestureRecognizerStateBegan) {
        ...
        WGPoint direction = WGPointMake(last.y - current.y, current.x - last.x, 0);
