...
#define TextColor RGB(73,139,251)
#define MAXYEAR 2100
#define MINYEAR 1870

typedef void(^doneBlock)(NSDate *);

@interface WGDatePickerView ()<UIPickerViewDelegate,UIPickerViewDataSource,UIGestureRecognizerDelegate> {
    //日期存储数组
...
@property (nonatomic,assign)WGDateStyle datePickerStyle;


@end

@implementation WGDatePickerView
...
-(void)setupUI {
    ...
    self.doneButtonColor = RGB(73,139,251);
...
-(void)defaultConfig {
    ...
    if (!self.maxLimitDate) {
        self.maxLimitDate = [NSDate date:@"2100-12-31 23:59" WithFormat:@"yyyy-MM-dd HH:mm"];
    }
    //最小限制
    if (!self.minLimitDate) {
        self.minLimitDate = [NSDate date:@"1870-01-01 00:00" WithFormat:@"yyyy-MM-dd HH:mm"];
    }
}
...
-(void)addLabelWithName:(NSArray *)nameArr {
    ...
    for (int i = 0; i<nameArr.count; i++) {
        CGFloat labelX = 0;
        if (i == 0) {
            labelX = kPickerSize.width/(nameArr.count*2)+18+kPickerSize.width/nameArr.count*i;
        }else{
            labelX = kPickerSize.width/(nameArr.count*2)+11+kPickerSize.width/nameArr.count*i;
        }
