...
@interface WGPolygonalRadarView ()

@property (nonatomic, weak  ) id<WGPolygonalRadarViewDataSource>     radarDataSource;
...
@property (nonatomic, strong) WGPolygonalRadarBgStyleConfig          *bgConfig;
@property (nonatomic, copy  ) NSArray<WGPolygonalRadarStyleConfig *> *radarConfig;

@property (nonatomic, strong) WGPolygonalRadarStyleConfig            *curRadarConfig;

@end

@implementation WGPolygonalRadarView
...
- (instancetype)initWithCenter:(CGPoint)center
    ...
    if (self) {
        ...
        for (WGPolygonalRadarStyleConfig *radarStyleConfig in _radarConfig) {
...
- (void)setupPolygonalBg {
    switch (_bgConfig.bgStyle) {
        case ENUM_WGPOLYGONALRADAR_BG_STYLE_DEFAULT:{
            //刻度线
        ...
        case ENUM_WGPOLYGONALRADAR_BG_STYLE_CUSTOM_IMAGE:{
    ...
    if (_bgConfig.showTitleLabel &&
        _radarDataSource &&
        [_radarDataSource respondsToSelector:@selector(WGPolygonalRadarTitleLabelAtIndex:)]) {
        for (NSInteger i = 0; i < _edgeCount; i++) {
            ...
            UILabel *titleLabel = [_radarDataSource WGPolygonalRadarTitleLabelAtIndex:i];
...
- (void)setupPolygonal {
    ...
    if (_curRadarConfig.showPolygonalFill) {
        ...
        if (ENUM_WGPOLYGONALRADAR_FILL_STYLE_SOLID_COLOR == _curRadarConfig.fillStyleConfig.fillStyle) {
            [self.layer addSublayer:fillShapeLayer];
        } else if (ENUM_WGPOLYGONALRADAR_FILL_STYLE_GRADIENT_COLOR == _curRadarConfig.fillStyleConfig.fillStyle) {
...
- (CAShapeLayer *)scaleLinesLayer {
    ...
    for (NSInteger i = _bgConfig.scaleLineCount - 1; i >= 0; i--) {
        ...
        //虚线情况
        ENUM_WGPOLYGONALRADAR_LINE_STYLE scaleLineStyle = [_bgConfig.scaleLineStyles[i] integerValue];
        if (ENUM_WGPOLYGONALRADAR_LINE_STYLE_DASH_LINE == scaleLineStyle &&
...
- (CAShapeLayer *)symmetryAxesLayerWithIndex:(NSInteger)index {
    ...
    if (ENUM_WGPOLYGONALRADAR_LINE_STYLE_DASH_LINE == _bgConfig.symmetryAxisStyle) {
...
- (CAShapeLayer *)polygonalBorderLayerWithBezier:(UIBezierPath *)polygonalBezier {
    ...
    if (ENUM_WGPOLYGONALRADAR_LINE_STYLE_DASH_LINE == _curRadarConfig.borderStyleConfig.borderStyle) {
...
- (CAShapeLayer *)vertexLayerWithIndex:(NSInteger)index {
    ...
    if (vertexseBezier) {
        ...
        if (ENUM_WGPOLYGONALRADAR_VERTEX_ICON_STYLE_SOLID_CIRCLE == _curRadarConfig.vertexStyleConfig.vertexStyle) {
            vertexShapeLayer.fillColor = _curRadarConfig.vertexStyleConfig.vertexColor.CGColor;
        } else if (ENUM_WGPOLYGONALRADAR_VERTEX_ICON_STYLE_HOLLOW_CIRCLE == _curRadarConfig.vertexStyleConfig.vertexStyle) {
...
- (UIBezierPath *)drawPolygonalBezier {
    ...
    if (_radarDataSource &&
        [_radarDataSource respondsToSelector:@selector(WGPolygonalRadarValueAtIndex:radarIndex:)]) {
        for (NSInteger i = 0; i < _edgeCount; i++) {
            CGFloat valueRadius = [_radarDataSource WGPolygonalRadarValueAtIndex:i radarIndex:[_radarConfig indexOfObject:_curRadarConfig]] * _radius;
...
- (UIBezierPath *)drawVertexWithIndex:(NSInteger)index {
    ...
    if (_radarDataSource &&
        [_radarDataSource respondsToSelector:@selector(WGPolygonalRadarValueAtIndex:radarIndex:)]) {
        CGFloat valueRadius = _radius * [_radarDataSource WGPolygonalRadarValueAtIndex:index radarIndex:[_radarConfig indexOfObject:_curRadarConfig]];
