...
+ (WGTimerDataSourceModel *)configDataSource:(NSInteger)kDays
    ...
    WGTimerDataSourceModel *dataSourceModel = [WGTimerDataSourceModel new];
    ...
    for (int i = 0; i < kDays; i++) {
        ...
        WGDateModel *model = [[WGDateModel alloc]init];
    ...
    for (NSInteger i = beginHour; i < 24 ; i++) {
        WGHourModel *model = [[WGHourModel alloc]init];
        if (i < 10) {
    ...
    for (NSInteger i = minStart; i < 60 / kTimeInterval; i++) {
        WGMinuteModel *model = [[WGMinuteModel alloc]init];
