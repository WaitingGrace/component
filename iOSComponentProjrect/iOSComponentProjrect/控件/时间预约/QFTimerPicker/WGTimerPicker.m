...
#import "WGTimerUtil.h"
#import "WGTimerDataSourceModel.h"
#import "WGDateModel.h"
#import "WGHourModel.h"
#import "WGMinuteModel.h"

@interface WGTimerPicker ()<UIPickerViewDataSource,UIPickerViewDelegate>
...
@property (nonatomic, strong)  WGTimerDataSourceModel *dataSourceModel;

@end

@implementation WGTimerPicker
...
- (instancetype)initWithDays:(NSInteger)days timeInterval:(NSInteger)timeInterval BenginTimeDely:(NSInteger)benginTimeDely Response:(ReturnBlock)block {
    if (self = [super init]) {
        [self initDataSourceWithDays:days timeInterval:timeInterval BenginTimeDely:benginTimeDely];
        [self initAppreaence];
...
- (void)initDataSourceWithDays:(NSInteger)days timeInterval:(NSInteger)timeInterval BenginTimeDely:(NSInteger)benginTimeDely {
    self.selectedDateIndex = 0;
    self.selectedHourIndex = 0;
    self.selectedMinuteIndex = 0;
    
    self.dataSourceModel = [WGTimerUtil configDataSource:days TimeInterval:timeInterval BenginTimeDely:benginTimeDely];
}
...
- (void)buttonAction:(UIButton *)sender {
    if (sender.tag == 100) {//取消
        ...
        if (self.returnBlock) {
            WGDateModel *dateModel = self.dataSourceModel.dateArray[self.selectedDateIndex];
        
            WGHourModel *hourModel;
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UILabel *)recycledLabel {
    ...
    if (component == 0) {
        WGDateModel *dateModel = self.dataSourceModel.dateArray[row];
