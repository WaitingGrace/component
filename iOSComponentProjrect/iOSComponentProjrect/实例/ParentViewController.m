...
@interface ParentViewController () <WGPageMenuDelegate, UIScrollViewDelegate>
@property (nonatomic, strong) NSArray *dataArr;
@property (nonatomic, weak) WGPageMenu *pageMenu;
...
// 示例1:WGPageMenuTrackerStyleLine,下划线样式
- (void)test1 {
    self.dataArr = @[@"生活",@"影视中心",@"交通",@"电视剧",@"搞笑",@"综艺"];

    // trackerStyle:跟踪器的样式
    WGPageMenu *pageMenu = [WGPageMenu pageMenuWithFrame:CGRectMake(0, NaviH, screenW, pageMenuH) trackerStyle:WGPageMenuTrackerStyleLine];
    // 传递数组，默认选中第2个
...
// 示例2:WGPageMenuTrackerStyleLineLongerThanItem,下划线比item略长，长度等于tem宽＋间距
// 这个例子采用自动布局
- (void)test2 {
    self.dataArr = @[@"生活",@"影视中心",@"交通",@"电视剧",@"搞笑",@"综艺"];
    
    // trackerStyle:跟踪器的样式
    WGPageMenu *pageMenu = [WGPageMenu pageMenuWithFrame:CGRectZero trackerStyle:WGPageMenuTrackerStyleLineLongerThanItem];
...
// 示例3:WGPageMenuTrackerStyleLineAttachment,下划线依恋样式，当滑动底下装载控制器的scrollView时，该下划线会有阻尼效果
- (void)test3 {
    self.dataArr = @[@"生活",@"影视中心",@"交通",@"电视剧",@"搞笑",@"综艺"];
    
    // trackerStyle:跟踪器的样式
    WGPageMenu *pageMenu = [WGPageMenu pageMenuWithFrame:CGRectMake(0, NaviH, screenW, pageMenuH) trackerStyle:WGPageMenuTrackerStyleLineAttachment];
    // 传递数组，默认选中第2个
...
// 示例4:WGPageMenuTrackerStyleTextZoom、WGPageMenuTrackerStyleNothing,缩放
- (void)test4 {
    self.dataArr = @[@"生活",@"影视中心",@"交通",@"电视剧",@"搞笑",@"综艺"];
    
    // trackerStyle:跟踪器的样式
    WGPageMenu *pageMenu = [WGPageMenu pageMenuWithFrame:CGRectMake(0, NaviH, screenW, pageMenuH) trackerStyle:WGPageMenuTrackerStyleNothing];
    // 传递数组，默认选中第1个
    [pageMenu setItems:self.dataArr selectedItemIndex:0];
    // 设置缩放
...
// 示例5:WGPageMenuTrackerStyleRoundedRect,圆角矩形
- (void)test5 {
    self.dataArr = @[@"生活",@"影视中心",@"交通",@"电视剧",@"搞笑",@"综艺"];
    
    // trackerStyle:跟踪器的样式
    WGPageMenu *pageMenu = [WGPageMenu pageMenuWithFrame:CGRectMake(0, NaviH, screenW, pageMenuH) trackerStyle:WGPageMenuTrackerStyleRoundedRect];
    // 传递数组，默认选中第1个
    [pageMenu setItems:self.dataArr selectedItemIndex:0];
...
// 示例6:WGPageMenuTrackerStyleRoundedRect,圆角矩形（与pageMenu同时圆角）
- (void)test6 {
    self.dataArr = @[@"生活",@"影视中心",@"交通"];

    // trackerStyle:跟踪器的样式
    WGPageMenu *pageMenu = [WGPageMenu pageMenuWithFrame:CGRectMake(15, NaviH, screenW-30, pageMenuH) trackerStyle:WGPageMenuTrackerStyleRoundedRect];
    // 设置pageMenu边框
    ...
    // 排列方式
    pageMenu.permutationWay = WGPageMenuPermutationWayNotScrollAdaptContent;
...
// 示例7:WGPageMenuTrackerStyleRect,矩形

- (void)test7{
    self.dataArr = @[@"生活",@"影视中心",@"交通",@"电视剧",@"搞笑",@"综艺"];
    
    // trackerStyle:跟踪器的样式
    WGPageMenu *pageMenu = [WGPageMenu pageMenuWithFrame:CGRectMake(0, NaviH, screenW, pageMenuH) trackerStyle:WGPageMenuTrackerStyleRect];
    // 传递数组，默认选中第2个
    [pageMenu setItems:self.dataArr selectedItemIndex:1];
    // 设置代理
    pageMenu.delegate = self;
...
// 示例9:可滑动的自适应内容排列，关键代码:pageMenu.permutationWay = WGPageMenuPermutationWayScrollAdaptContent;
- (void)test9 {
    self.dataArr = @[@"生活",@"影视中心",@"交通",@"电视剧",@"搞笑",@"综艺"];
    ...
    [pageMenu setItems:self.dataArr selectedItemIndex:1];
    // 可滑动的自适应内容排列
    pageMenu.permutationWay = WGPageMenuPermutationWayScrollAdaptContent;
    // 设置代理
    pageMenu.delegate = self;
    // 给pageMenu传递外界的大scrollView，内部监听self.scrollView的滚动，从而实现让跟踪器跟随self.scrollView移动的效果
    pageMenu.bridgeScrollView = self.scrollView;
...
// 示例10:不可滑动的等宽排列，关键代码:pageMenu.permutationWay = WGPageMenuPermutationWayNotScrollEqualWidths;
- (void)test10 {
    ...
    [pageMenu setItems:self.dataArr selectedItemIndex:1];
    // 不可滑动的等宽排列
    pageMenu.permutationWay = WGPageMenuPermutationWayNotScrollEqualWidths;
...
// 示例11:不可滑动的自适应内容排列，关键代码:pageMenu.permutationWay = WGPageMenuPermutationWayNotScrollAdaptContent;
...
- (void)test12 {
    ...
    // 传递数组，默认选中第2个
    [pageMenu setItems:self.dataArr selectedItemIndex:0];
    pageMenu.trackerFollowingMode = WGPageMenuTrackerFollowingModeAlways;
    // 设置代理
    pageMenu.delegate = self;
    // 给pageMenu传递外界的大scrollView，内部监听self.scrollView的滚动，从而实现让跟踪器跟随self.scrollView移动的效果
    pageMenu.bridgeScrollView = self.scrollView;
...
- (void)test13 {
    ...
    // 传递数组，默认选中第2个
    [pageMenu setItems:self.dataArr selectedItemIndex:0];
    pageMenu.trackerFollowingMode = WGPageMenuTrackerFollowingModeEnd;
    // 设置代理
    pageMenu.delegate = self;
    // 给pageMenu传递外界的大scrollView，内部监听self.scrollView的滚动，从而实现让跟踪器跟随self.scrollView移动的效果
    pageMenu.bridgeScrollView = self.scrollView;
...
- (void)test14 {
    ...
    // 传递数组，默认选中第2个
    [pageMenu setItems:self.dataArr selectedItemIndex:0];
    pageMenu.trackerFollowingMode = WGPageMenuTrackerFollowingModeHalf;
    // 设置代理
    pageMenu.delegate = self;
    // 给pageMenu传递外界的大scrollView，内部监听self.scrollView的滚动，从而实现让跟踪器跟随self.scrollView移动的效果
    pageMenu.bridgeScrollView = self.scrollView;
...
- (void)test16 {
    ...
    [pageMenu setFunctionButtonContent:[WGPageMenuButtonItem itemWithTitle:@"更多" image:[UIImage imageNamed:@"Expression_1"] imagePosition:WGItemImagePositionTop] forState:UIControlStateNormal];
...
- (void)test17 {
    ...
    WGPageMenu *pageMenu = [WGPageMenu pageMenuWithFrame:CGRectMake(0, NaviH, screenW, pageMenuH) trackerStyle:WGPageMenuTrackerStyleLineLongerThanItem];
    // 传递数组，默认选中第2个
    [pageMenu setItems:self.dataArr selectedItemIndex:1];
    pageMenu.delegate = self;
    // 给pageMenu传递外界的大scrollView，内部监听self.scrollView的滚动，从而实现让跟踪器跟随self.scrollView移动的效果
...
- (void)test18 {
    ...
    WGPageMenuButtonItem *item1 = [WGPageMenuButtonItem itemWithTitle:@"害羞" image:[UIImage imageNamed:@"Expression_2"]];
    item1.imagePosition = WGItemImagePositionTop;
    [pageMenu setItem:item1 forItemAtIndex:1];
    // 指定第4个item同时含有图片和文字，图片在右
//    [pageMenu setTitle:@"可爱的小狗" image:[UIImage imageNamed:@"dog"] imagePosition:WGItemImagePositionDefault imageRatio:0.4 imageTitleSpace:0 forItemIndex:3];
    WGPageMenuButtonItem *item2 = [WGPageMenuButtonItem itemWithTitle:@"歌曲" image:[UIImage imageNamed:@"asc"] imagePosition:WGItemImagePositionRight];
...
- (void)test21 {
    ...
    WGBadgeView *badgeView0 = [[WGBadgeView alloc] initWithParentView:button0.titleLabel alignment:WGBadgeViewAlignmentTopRight];
...
- (void)pageMenu:(WGPageMenu *)pageMenu itemSelectedFromIndex:(NSInteger)fromIndex toIndex:(NSInteger)toIndex {
    ...
    // 如果该代理方法是由拖拽self.scrollView而触发，说明self.scrollView已经在用户手指的拖拽下而发生偏移，此时不需要再用代码去设置偏移量，否则在跟踪模式为WGPageMenuTrackerFollowingModeHalf的情况下，滑到屏幕一半时会有闪跳现象。闪跳是因为外界设置的scrollView偏移和用户拖拽产生冲突
...
// object是插入的对象(NSString、UIImage或WGPageMenuButtonItem),insertNumber是插入到第几个
