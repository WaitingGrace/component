...
@interface WGOutPutView () <UITableViewDataSource, UITableViewDelegate>
...
@property (nonatomic, assign) WGOutputViewDirection direction;

@end
@implementation WGOutPutView
...
- (instancetype)initWithDataArray:(NSArray *)dataArray
    ...
    if (self = [super initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)]) {
        ...
        if (direction == kWGOutputViewDirectionLeft) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    //取出模型
    WGCellModel *model = [self.dataArray objectAtIndex:indexPath.row];
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.direction == kWGOutputViewDirectionLeft) {
...
#pragma mark - WGCellModel

@implementation WGCellModel

- (instancetype)initWithTitle:(NSString *)title imageName:(NSString *)imageName {
    WGCellModel *model = [[WGCellModel alloc] init];
    model.title = title;
